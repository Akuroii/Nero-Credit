...
export type GradientOrbConfig = {
  ...
  innerRadius?: number;
  /**
   * Device pixel ratio cap. Small orbs don't need full retina density;
   * capping this keeps many simultaneous orb instances cheap.
   * @default 1.5
   */
  maxDpr?: number;
};
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    vec4 col = draw(uv);
    // Premultiplied alpha, composited transparently over the page —
    // no opaque backing plate behind the orb.
...
/**
 * Glowing shader orb, rendered with a transparent canvas so it composites
 * directly onto whatever sits behind it (an avatar, a page background)
 * rather than punching an opaque square into the scene.
 *
 * @example
 * ```tsx
 * <GradientOrb className="h-40 w-40" />
...
export function GradientOrb({
  ...
  className?: string;
  /** Freezes the frameloop — used for prefers-reduced-motion and off-screen orbs. */
  paused?: boolean;
}) {
  const config = useMemo(
    ...
    [
      ...
      configOverrides?.noiseScale,
      configOverrides?.innerRadius,
      configOverrides?.maxDpr,
    ],
  );
